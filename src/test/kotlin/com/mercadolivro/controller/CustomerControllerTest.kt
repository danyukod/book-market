...
class CustomerControllerTest{
    ...
    fun `should filter customers by name when get all`() {
        val customer1 = customerRepository.save(buildCustomer(name = "Danilo"))
        val customer2 = customerRepository.save(buildCustomer(name = "Daniel"))
        val customer3 = customerRepository.save(buildCustomer(name = "Thomas"))

        mockMvc.perform(get("/customers?name=Dan"))
            .andExpect(status().isOk)
            .andExpect(jsonPath("$.length()").value(2))
            .andExpect(jsonPath("$[0].id").value(customer1.id))
            .andExpect(jsonPath("$[0].name").value(customer1.name))
    ...
    fun `should update customer`() {
        val customer = customerRepository.save(buildCustomer())
        val request = PutCustomerRequest("Danilo", "emailupdate@email.com")
    ...
    @Test
    fun `should return not found when delete customer not existing`() {
